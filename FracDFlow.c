...
#undef __FUNCT__
#define __FUNCT__ "FracDUnPackSgRsFieldIntoSgAndRs"
extern PetscErrorCode FracDUnPackSgRsFieldIntoSgAndRs(Vec Rs, Vec Sg, Vec Pb, Vec P, Vec SgRs, Vec INDC2,  FracDPbRs SolutionGasOilData)
{
    PetscErrorCode      ierr;
    DM                  dm;
    PetscInt            v,vStart,vEnd,offset;
    PetscSection        localSection;
    PetscScalar         *IND2_array,*Sg_array,*Rs_array,*SgRs_array,*P_array,*Pb_array;
    Vec                 local_IND2,local_Sg,local_Rs,local_SgRs,local_P,local_Pb;
    PetscReal           pb_value;
    ...
    ierr = DMGetLocalVector(dm,&local_SgRs);CHKERRQ(ierr);
    ierr = VecSet(local_SgRs,0.);CHKERRQ(ierr);
    ierr = DMGlobalToLocalBegin(dm,SgRs,INSERT_VALUES,local_SgRs);CHKERRQ(ierr);
    ierr = DMGlobalToLocalEnd(dm,SgRs,INSERT_VALUES,local_SgRs);CHKERRQ(ierr);
    ierr = VecGetArray(local_SgRs,&SgRs_array);CHKERRQ(ierr);
    ...
    for(v = vStart; v < vEnd; ++v){
        ierr = PetscSectionGetOffset(localSection, v, &offset);CHKERRQ(ierr);
        if( PetscAbs(IND2_array[offset]-1.) < PETSC_SMALL || PetscAbs(IND2_array[offset]-4.) < PETSC_SMALL){
            Sg_array[offset] = SgRs_array[offset];
            ierr = SolutionGasOilData.FracDUpDateSolutionGasOilRatio(&rs_value,P_array[offset],PETSC_NULL,SolutionGasOilData.TableData,PETSC_NULL,SolutionGasOilData.numdatarow);CHKERRQ(ierr);
            Rs_array[offset] = rs_value;
        }
        if( PetscAbs(IND2_array[offset]-2.) < PETSC_SMALL || PetscAbs(IND2_array[offset]-3.) < PETSC_SMALL){
            Rs_array[offset] = SgRs_array[offset];
            Sg_array[offset] = 0.;
        }
        pb_value = 0;
        ierr = SolutionGasOilData.FracDUpDateBubblePoint(&pb_value,Rs_array[offset],PETSC_NULL,SolutionGasOilData.TableDataInv,PETSC_NULL,SolutionGasOilData.numdatarow);CHKERRQ(ierr);
    ...
    ierr = VecRestoreArray(local_Sg,&Sg_array);CHKERRQ(ierr);
    ierr = DMLocalToGlobalBegin(dm,local_Sg,INSERT_VALUES,Sg);CHKERRQ(ierr);
    ierr = DMLocalToGlobalEnd(dm,local_Sg,INSERT_VALUES,Sg);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(dm,&local_Sg);CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
...
#undef __FUNCT__
#define __FUNCT__ "FracDSwitchVariables"
extern PetscErrorCode FracDSwitchVariables(Vec INDC1, Vec INDC2, Vec Rs, Vec P, Vec Pb, Vec Sw, Vec Sg,  FracDPVT OilPVTData, FracDPVT GasPVTData, FracDPbRs            SolutionGasOilData)
{
    PetscErrorCode      ierr;
    DM                  dm;
    PetscInt            v,vStart,vEnd,offset;
    ...
    for(v = vStart; v < vEnd; ++v){
        ierr = PetscSectionGetOffset(localSection, v, &offset);CHKERRQ(ierr);
        ierr = SolutionGasOilData.FracDUpDateSolutionGasOilRatio(&rs_value,P_array[offset],PETSC_NULL,SolutionGasOilData.TableData,PETSC_NULL,SolutionGasOilData.numdatarow);CHKERRQ(ierr);
        if( PetscAbs(Rs_array[offset]-rs_value) > PETSC_SMALL){
            IND1_array[offset] = 1.;
            if( PetscAbs(IND2_array[offset]-3.) < PETSC_SMALL)  IND2_array[offset] = 4.;
            if( PetscAbs(IND2_array[offset]-2.) < PETSC_SMALL)  IND2_array[offset] = 1.;
