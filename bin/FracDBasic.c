...
extern PetscErrorCode FracDCreateFields(AppCtx *bag,FracDFields *fields)
{
    ...
    ierr = DMCreateGlobalVector(bag->WellRedun, &fields->INDC1);CHKERRQ(ierr);
    ...
    ierr = DMCreateGlobalVector(bag->WellRedun, &fields->INDC2);CHKERRQ(ierr);
    ...
    ierr = PetscOptionsBegin(PETSC_COMM_WORLD, PETSC_NULL, "Initial state of the reservoir", "");CHKERRQ(ierr);
    {
        ...
        if (bag->fluid != SINGLEPHASELIQUID){
            ...
            if(!rank){
                ...
                fclose(filename);
                ini_Rs = ini_Rs/bag->ppties.SolutionGasOilData.RateConversion;
            }
