...
extern PetscErrorCode FracDUpdateSolutionGasOilRatioAndDerivative(Vec Rs, Vec dervRs, Vec Pb, Vec P, PetscReal delta, PetscReal fixed_value, PetscReal *model_data, PetscReal **TableData, PetscInt numdatarow)
{
    ...
    for(v = vStart; v < vEnd; ++v){
        ...
        ierr =  FracDQuantityAndDerivativeComputation(RsData,P_array[offset],delta,Pb_array[offset],TableData,model_data,numdatarow,FracDInterpolateUsingAnalyticalAndCubicSpline);CHKERRQ(ierr);
...
extern PetscErrorCode FracDInitializeSolutionGasOilRatio(Vec Rs, Vec P, Vec Pb, Vec Sw, Vec Sg, FracDPVT OilPVTData, FracDPVT GasPVTData, PetscReal ini_Rs)
{
    PetscErrorCode      ierr;
    DM                  dm;
    PetscInt            v,vStart,vEnd,offset;
    PetscSection        localSection;
    PetscScalar         *Rs_array,*Sw_array,*Sg_array, *P_array, *Pb_array;
    Vec                 local_Rs, local_Sw, local_Sg, local_P, local_Pb;
    PetscReal           So,Bo,Bg;
