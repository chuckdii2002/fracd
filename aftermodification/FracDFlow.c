...
extern PetscErrorCode FracDPResidual(SNES snesP,Vec V,Vec residual,void *user)
{
    ...
    switch (bag->fluid) {
        ...
        case 3:
        {
            ...
            ierr = FracDUpdateBubblePoint(bag->fields.Pb,bag->fields.SaturatedStateIndicator,P,bag->ppties.BubblePointFixed);CHKERRQ(ierr);
            ierr = FracDUpdateSolutionGasOilRatioAndDerivative(bag->fields.Rs,bag->fields.dervRs,bag->fields.Pb,P,bag->SMALL_PRESSURE,bag->ppties.SolutionGasOilRatioFixed,bag->ppties.SolutionGasOilData.ModelData,bag->ppties.SolutionGasOilData.TableData,bag->ppties.SolutionGasOilData.numdatarow);CHKERRQ(ierr);
...
extern PetscErrorCode FracDUpdateSolutionGasOilRatioAndDerivative(Vec Rs, Vec dervRs, Vec Pb, Vec P, PetscReal delta, PetscReal fixed_value, PetscReal *model_data, PetscReal **TableData, PetscInt numdatarow)
{
    ...
    for(v = vStart; v < vEnd; ++v){
        ierr = PetscSectionGetOffset(localSection, v, &offset);CHKERRQ(ierr);
        model_data[2] = P_array[offset];
        ierr =  FracDQuantityAndDerivativeComputation(RsData,P_array[offset],delta,PETSC_NULL,TableData,model_data,numdatarow,FracDInterpolateUsingAnalyticalAndCubicSpline);CHKERRQ(ierr);
...
extern PetscErrorCode FracDInitializeSolutionGasOilRatio(Vec Rs, Vec P, Vec Sw, Vec Sg, FracDPVT OilPVTData, FracDPVT GasPVTData, PetscReal ini_Rs)
{
    PetscErrorCode      ierr;
    DM                  dm;
    PetscInt            v,vStart,vEnd,offset;
    ...
    ierr = DMGetLocalVector(dm,&local_P);CHKERRQ(ierr);
    ierr = VecSet(local_P,0.);CHKERRQ(ierr);
    ierr = DMGlobalToLocalBegin(dm,Sg,INSERT_VALUES,local_P);CHKERRQ(ierr);
    ierr = DMGlobalToLocalEnd(dm,Sg,INSERT_VALUES,local_P);CHKERRQ(ierr);
    ierr = VecGetArray(local_P,&P_array);CHKERRQ(ierr);
    
    for(v = vStart; v < vEnd; ++v){
        ierr = PetscSectionGetOffset(localSection, v, &offset);CHKERRQ(ierr);
        So = 1.-Sw_array[offset]-Sg_array[offset];
        ierr =  OilPVTData.FracDUpDateFVF(&Bo,P_array[offset],PETSC_NULL,OilPVTData.B_TableData,OilPVTData.B_ModelData,OilPVTData.numdatarow);CHKERRQ(ierr);
