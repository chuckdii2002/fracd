/*
 FracD: Initialize the FracD code. Called by the fortran implementation of VIADAT
 
 (c) 2016-2016 Chukwudi Chukwudozie chdozie@gmail.com
 */
...
extern PetscErrorCode FracDProcessPVTData(AppCtx *bag)
{
    ...
    bag->ppties.SolutionGasOilData.ModelData[0] = bag->ppties.SolutionGasOilRatioFixed;
    bag->ppties.SolutionGasOilData.ModelData[1] = bag->ppties.SolutionGasOilData.ModelData[1] = 0.;
    ...
    if (bag->fluid == OILWATERGAS){
        ...
        bag->ppties.SolutionGasOilData.ModelData[0] = 0.;
        bag->ppties.SolutionGasOilData.numdatarow = 2;
        bag->ppties.SolutionGasOilData.TableData = (PetscReal **)malloc(3 * sizeof(PetscReal *));
        for(i = 0; i < 3; i++){
            bag->ppties.SolutionGasOilData.TableData[i] = (PetscReal *)malloc(bag->ppties.SolutionGasOilData.numdatarow * sizeof(PetscReal));
        }
        for(i = 0; i < bag->ppties.SolutionGasOilData.numdatarow; i++){
            bag->ppties.SolutionGasOilData.TableData[0][i] = bag->ppties.SolutionGasOilData.TableData[1][i] = bag->ppties.SolutionGasOilData.TableData[2][i] = 0.;
        }
    }
    bag->ppties.SolutionGasOilData.FracDUpDateSolutionGasOilRatio = FracDInterpolateUsingAnalyticalAndCubicSpline;
    
    
    
    bag->SMALL_SATURATION    = 1e-2;
    bag->SMALL_PRESSURE      = 1e-6;
    if (bag->fluid == OILWATERGAS || bag->fluid == OILWATER){
...
extern PetscErrorCode FracDCreateFEShapeFunction(AppCtx *bag)
{
    ...
    switch (bag->dim) {
        ...
        if  (bag->elementType == TRIANGLE){
            bag->nodes = 3;
            bag->FracDCreateCVFEFace = FracD2DCVFEFace;
        ...
        if  (bag->elementType == TETRAHEDRAL){
            ...
            bag->FracDCreateCVFEFace = FracD3DCVFEFace;
...
extern PetscErrorCode FracDFinalizePVTData(AppCtx *bag)
{
    ...
    if(bag->fluid == OILWATER || bag->fluid == OILWATERGAS){
        if(bag->ppties.OilPVTData.FVFtype == INTERPOLATION || bag->ppties.OilPVTData.mutype == INTERPOLATION || bag->ppties.OilPVTData.rhotype == INTERPOLATION){
            for(i = 0; i < 3; i++){
...
extern PetscErrorCode ThisismyVTKwriter(AppCtx *bag)
{
    ...
    ierr = PetscViewerFileSetName(viewer, "outputtest1.vtk");CHKERRQ(ierr);
    //    ierr = myWriteFunc((PetscObject) bag->fields.P, viewer);
    ierr = PetscViewerVTKAddField(viewer, (PetscObject) bag->plexScalNode, myWriteFunc, PETSC_VTK_POINT_FIELD, (PetscObject) bag->fields.P);CHKERRQ(ierr);
...
extern PetscErrorCode FracDTimeStepPrepare(AppCtx *bag)
{
    ...
    {
        ini_pb  = 0;
        if (bag->fluid != SINGLEPHASELIQUID){
            bag->ppties.solutiongas_bubblepoint_filename[0] = '\0';
            ierr = PetscOptionsString("-bubblepointfilename", "Solution gas oil ratio and bubble point data file name", "Name of Rs and Pb file to be imported", bag->ppties.solutiongas_bubblepoint_filename, bag->ppties.solutiongas_bubblepoint_filename, PETSC_MAX_PATH_LEN, PETSC_NULL);CHKERRQ(ierr);
            if(!rank){
                ierr   = PetscFOpen(PETSC_COMM_SELF,bag->ppties.solutiongas_bubblepoint_filename,"r",&filename);CHKERRQ(ierr);CHKERRQ(ierr);
                fscanf(filename, "%le%le", (double*)&ini_Rs, (double*)&ini_pb);
                fclose(filename);
            }
            MPI_Bcast(&ini_Rs,1,MPIU_REAL,0,PETSC_COMM_WORLD);
            MPI_Bcast(&ini_pb,1,MPIU_REAL,0,PETSC_COMM_WORLD);
        }
        bag->ppties.BubblePointFixed = ini_pb;
        bag->ppties.SolutionGasOilRatioFixed = ini_Rs;
        
//        bag->fields.Pb
        
//        printf("CHECKING RS:: %g %g \n\n\n\n\n",ini_pb,ini_Rs);
    ...
    if(bag->P_initial > bag->ppties.BubblePointFixed){
    ...
    ierr = VecSet(bag->fields.oPb,ini_pb);CHKERRQ(ierr);
    ierr = VecSet(bag->fields.oRs,ini_Rs);CHKERRQ(ierr);
