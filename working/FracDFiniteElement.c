...
extern PetscErrorCode FracDPointFEElementCreate(PetscInt dim, FracDPointFEElement *e, FracDElementType elementType)
{
    ...
    switch (dim) {
        ...
        case 2:
            ...
            if(elementType == QUADRILATERAL)    e->nodes     = 4;
        ...
        case 3:
            ...
            if(elementType == HEXAHEDRAL)       e->nodes     = 8;
            break;
        default:
...
#undef __FUNCT__
#define __FUNCT__ "FracD2DCVFEFace"
extern PetscErrorCode FracD2DCVFEFace(PetscReal **elemcoords, PetscReal **coords, FracDCVFEFace *f)
{
    PetscInt            i,j,k,l;
...
#undef __FUNCT__
#define __FUNCT__ "FracD3DCVFEFace"
extern PetscErrorCode FracD3DCVFEFace(PetscReal **elemcoords, PetscReal **coords, FracDCVFEFace *f)
{
    PetscInt            i,j,k,l;
                ...
                if( pointID[j] >= fStart && pointID[j] < fEnd){
                    ...
                    for(kk = 0; kk < numclpts; kk++){
                        ...
                        if(pt >= vStart && pt < vEnd){
                            ierr = PetscSectionGetOffset(section, pt, &offset);CHKERRQ(ierr);
                            for(k = 0; k < dof; k++){
                                Residual_array[offset+k] += dt * scale * elD->Volume * neumvalues[k];
                            }
                        }
                    }
                    ierr = DMPlexRestoreTransitiveClosure(dm, pointID[j], PETSC_TRUE, &numclpts, &closurept);CHKERRQ(ierr);
