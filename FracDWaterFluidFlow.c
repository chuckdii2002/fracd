...
extern PetscErrorCode FracDdRw_dP(void *user, Mat K, Mat KPC, Vec P, Vec Sw, Vec Sg, Vec Pbh)
{
    ...
    PetscScalar    *P1_array=NULL,*Sw1_array=NULL;
    PetscReal      dt,theta,effectiveCellPerm;
    PetscReal      G,lambda,phi,phiData[3],fpress;
    PetscReal      Bw,BwData[3],muw,muwData[3],krw,Qw,Qw1,dervQw,kr_check,kr[2];
    PetscReal      invBw_derv,invmuw_derv,phi_derv;
...
//        for(c = 17; c < 18; ++c){
        ...
        for(k = 0; k < numclpts; k++){
            ...
            if(pt >= gStart && pt < gEnd){
                ...
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr[0],Sw1_array[0],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr[1],Sw1_array[1],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = FracDQuantityAndDerivativeComputation(BwData,fpress,bag->SMALL_PRESSURE,PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow,bag->ppties.WaterPVTData.FracDUpDateFVF);CHKERRQ(ierr);
                ierr = FracDQuantityAndDerivativeComputation(muwData,fpress,bag->SMALL_PRESSURE,PETSC_NULL,bag->ppties.WaterPVTData.mu_TableData,bag->ppties.WaterPVTData.mu_ModelData,bag->ppties.WaterPVTData.numdatarow,bag->ppties.WaterPVTData.FracDUpDateViscosity);CHKERRQ(ierr);
                Bw = BwData[0]; invBw_derv = BwData[2];
                muw = muwData[0]; invmuw_derv = muwData[2];
                ...
                for(i = 0; i < rnumclpts1; i++){
                    ...
                    if(rpt1 >= vStart && rpt1 < vEnd){
                        ...
                        for (cj = 0; cj < bag->CVFEface.elemnodes; cj++)   kr_check += K_local[cj] * (P_array[cj]-Pcow_array[cj]);
                        if(kr_check > 0) krw = kr[jj];
                        else    krw = kr[(jj+1)%2];
                        lambda = krw/(muw*Bw);
                        ...
                        for (cj = 0; cj < bag->CVFEface.elemnodes; cj++) {
                            matvalue += CFbeta * theta * lambda * (Bw * invBw_derv + muw * invmuw_derv) * K_local[cj] * (P_array[cj]-Pcow_array[cj]);
                        }
        ...
        for(i = 0; i < bag->CVFEface.elemnodes; i++){
            ierr = FracDQuantityAndDerivativeComputation(BwData,P_array[i],bag->SMALL_PRESSURE,PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow,bag->ppties.WaterPVTData.FracDUpDateFVF);CHKERRQ(ierr);
            Bw = BwData[0]; invBw_derv = BwData[2];
            ...
            matvalue = (1./dt) * 1./CFalpha * scale * bag->CVFEface.elemVolume * Sw_array[i]*(phi*invBw_derv+phi_derv/Bw);
    ...
    for(i = 0; i < bag->WellinMeshData.numberWellsInProcessor; i++) {
        ...
        for(l = 0; l < bag->CVFEface.elemnodes; l++){
            ...
            ierr = FracDQw(&Qw,bag->well[w],G,Pbh_array[w],P_array[l],Pb_array[l],Sw_array[l],Sg_array[l],Pcow_array[l],Pcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            ierr = FracDQw(&Qw1,bag->well[w],G,Pbh_array[w],P_array[l]+bag->SMALL_PRESSURE,Pb_array[l],Sw_array[l],Sg_array[l],Pcow_array[l],Pcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            dervQw = (Qw1-Qw)/bag->SMALL_PRESSURE;
            matvalue =  -1. * theta * bag->epD.phi[l] * dervQw;
    ...
    ierr = FracDMatrixApplyDirichletBC(bag->plexScalNode, K,&bag->PBC,one);CHKERRQ(ierr);
    ierr = MatAssemblyBegin(K,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    ierr = MatAssemblyEnd(K,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    if (KPC != K) {
        ierr = MatAssemblyBegin(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
        ierr = MatAssemblyEnd(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
        ierr = FracDMatrixApplyDirichletBC(bag->plexScalNode, KPC,&bag->PBC,one);CHKERRQ(ierr);
        ierr = MatAssemblyBegin(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
        ierr = MatAssemblyEnd(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    }
    ...
    /*
     PetscViewer viewer;
     ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"MatrixP1.txt",&viewer);CHKERRQ(ierr);
     ierr = MatView(K,viewer);CHKERRQ(ierr);
     ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"MatrixPPCU1.txt",&viewer);CHKERRQ(ierr);
...
extern PetscErrorCode FracDdRw_dSw(void *user, Mat K, Mat KPC, Vec P, Vec Sw, Vec Sg, Vec Pbh)
{
    ...
    PetscInt       *closurept=NULL,*rclosurept=NULL,*rclosurept1=NULL;
    PetscReal      *cellcentroid,*edgecentroid,**facecentroid,**CVfacecoords,*wellcoords;
    Vec            localP,localPb,localPcow,localPcog,localdervPcow,localSw,localSg,localPbh;
    PetscScalar    *P_array=NULL,*Pb_array=NULL,*Pcow_array=NULL,*Pcog_array=NULL,*dervPcow_array=NULL,*Sw_array=NULL,*Sg_array=NULL,*Pbh_array=NULL;
    PetscScalar    *P1_array=NULL,*Sw1_array=NULL;
    PetscReal      dt,theta,effectiveCellPerm;
    PetscReal      G,lambda,phi,fpress;
    PetscReal      Bw,muw,krw,krw1,krw_derv,Qw,Qw1,dervQw,kr_check,kr[2],kr1[2];
    PetscReal      CFbeta, CFalpha, CFgamma;
    PetscErrorCode      (*FracDProjectFaceCoordinateDimensions)(PetscReal**,PetscReal**, PetscInt, PetscInt) = NULL;
    ...
    ierr = DMGetLocalVector(bag->plexScalNode,&localdervPcow);CHKERRQ(ierr);
    ierr = DMGlobalToLocalBegin(bag->plexScalNode,bag->fields.dervPcow,INSERT_VALUES,localdervPcow);CHKERRQ(ierr);
    ierr = DMGlobalToLocalEnd(bag->plexScalNode,bag->fields.dervPcow,INSERT_VALUES,localdervPcow);CHKERRQ(ierr);
    ...
    for(c = cStart; c < cEnd; ++c){
        ...
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localPcow, c, NULL, &Pcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localPcog, c, NULL, &Pcog_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localdervPcow, c, NULL, &dervPcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localSg, c, NULL, &Sg_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localSw, c, NULL, &Sw_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, local_CV, c, NULL, &CV_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(cdm, cordSection, coordinates, c, NULL, &coord_array);CHKERRQ(ierr);
        ...
        for(k = 0; k < numclpts; k++){
            ...
            if(pt >= gStart && pt < gEnd){
                ...
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr[0],Sw1_array[0],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr[1],Sw1_array[1],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr1[0],Sw1_array[0]+bag->SMALL_SATURATION,PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr1[1],Sw1_array[1]+bag->SMALL_SATURATION,PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.WaterPVTData.FracDUpDateFVF(&Bw,fpress,PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.WaterPVTData.FracDUpDateViscosity(&muw,fpress,PETSC_NULL,bag->ppties.WaterPVTData.mu_TableData,bag->ppties.WaterPVTData.mu_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
                ierr = DMPlexGetTransitiveClosure(bag->plexScalNode, pt, PETSC_TRUE, &rnumclpts1, &rclosurept1);CHKERRQ(ierr);
                jj = 0;
                for(i = 0; i < rnumclpts1; i++){
                    rpt1 = rclosurept1[2*i];
                    ...
                    if(rpt1 >= vStart && rpt1 < vEnd){
                        ...
                        lambda = krw/(muw*Bw);
                        krw_derv = (krw1-krw)/(bag->SMALL_SATURATION);
                        matvalue = 0;
                        for (cj = 0; cj < bag->CVFEface.elemnodes; cj++) {
                            matvalue += CFbeta * theta * K_local[cj] * (krw_derv/(muw*Bw) * (P_array[cj]-Pcow_array[cj]) - lambda * dervPcow_array[cj]);
                        }
                        ierr = MatSetValues(K, 1, rows, 1, rows, &matvalue, ADD_VALUES);CHKERRQ(ierr);
                        if (KPC != K) {
                            ierr = MatSetValues(KPC, 1, rows, 1, rows, &matvalue, ADD_VALUES);CHKERRQ(ierr);
        ...
        for(i = 0; i < bag->CVFEface.elemnodes; i++){
            ierr =  bag->ppties.WaterPVTData.FracDUpDateFVF(&Bw,P_array[l],PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
            ierr = FracDInterpolateUsingAnalyticalModel(&phi,P_array[i],PETSC_NULL,PETSC_NULL,bag->ppties.PhiData,PETSC_NULL);CHKERRQ(ierr);
            matvalue = (1./dt) * 1./CFalpha * scale * bag->CVFEface.elemVolume * phi/Bw;
            ierr = MatSetValues(K, 1, &cols[i], 1, &cols[i], &matvalue, ADD_VALUES);CHKERRQ(ierr);
            if (KPC != K) {
                ierr = MatSetValues(KPC, 1, &cols[i], 1, &cols[i], &matvalue, ADD_VALUES);CHKERRQ(ierr);
            }
        ...
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localPcow, c, NULL, &Pcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localPcog, c, NULL, &Pcog_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localdervPcow, c, NULL, &dervPcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localSg, c, NULL, &Sg_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localSw, c, NULL, &Sw_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, local_CV, c, NULL, &CV_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(cdm, cordSection, coordinates, c, NULL, &coord_array);CHKERRQ(ierr);
    ...
    for(i = 0; i < bag->WellinMeshData.numberWellsInProcessor; i++) {
        ...
        for(l = 0; l < bag->CVFEface.elemnodes; l++){
            ...
            G = 2*PETSC_PI*CFbeta*effectiveCellPerm*bag->well[w].h/(PetscLogReal(bag->well[w].re/bag->well[w].rw)+bag->well[w].sk);
            ierr = FracDQw(&Qw,bag->well[w],G,Pbh_array[w],P_array[l],Pb_array[l],Sw_array[l],Sg_array[l],Pcow_array[l],Pcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            ierr = FracDQw(&Qw1,bag->well[w],G,Pbh_array[w],P_array[l],Pb_array[l],Sw_array[l]+bag->SMALL_SATURATION,Sg_array[l],Pcow_array[l],Pcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            dervQw = (Qw1-Qw)/bag->SMALL_SATURATION;
            matvalue =  -1. * theta * bag->epD.phi[l] * dervQw;
            ierr = MatSetValues(K, 1, &cols[l], 1, &cols[l], &matvalue, ADD_VALUES);CHKERRQ(ierr);
            if (KPC != K) {
                ierr = MatSetValues(KPC, 1, &cols[l], 1, &cols[l], &matvalue, ADD_VALUES);CHKERRQ(ierr);
    ...
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localPcow);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localPcog);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localdervPcow);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localSg);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localSw);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(bag->plexScalNode,&local_CV);CHKERRQ(ierr);
    ierr = VecRestoreArray(localPbh,&Pbh_array);CHKERRQ(ierr);
    ...
    ierr = MatAssemblyBegin(K,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    ierr = MatAssemblyEnd(K,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    ierr = FracDMatrixApplyDirichletBC(bag->plexScalNode, K,&bag->PBC,zero);CHKERRQ(ierr);
    ierr = MatAssemblyBegin(K,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    ierr = MatAssemblyEnd(K,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    if (KPC != K) {
        ierr = MatAssemblyBegin(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
        ierr = MatAssemblyEnd(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
        ierr = FracDMatrixApplyDirichletBC(bag->plexScalNode, KPC,&bag->PBC,zero);CHKERRQ(ierr);
        ierr = MatAssemblyBegin(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
        ierr = MatAssemblyEnd(KPC,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
    }
    ierr = PetscFree4(cols,rows,K_local,K1_local);CHKERRQ(ierr);
    ...
    /*
    PetscViewer viewer;
    ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"MatrixP2.txt",&viewer);CHKERRQ(ierr);
    ierr = MatView(K,viewer);CHKERRQ(ierr);
    ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"MatrixPPCU2.txt",&viewer);CHKERRQ(ierr);
    ierr = MatView(KPC,viewer);CHKERRQ(ierr);
    */
    PetscFunctionReturn(0);
}
...
#undef __FUNCT__
#define __FUNCT__ "FracDdRw_dPbh"
extern PetscErrorCode FracDdRw_dPbh(void *user, Mat K, Mat KPC, Vec P, Vec Sw, Vec Sg, Vec Pbh)
{
    ...
    PetscScalar    *P_array=NULL,*Pb_array=NULL,*Pcow_array=NULL,*Pcog_array=NULL,*Sw_array=NULL,*Sg_array=NULL,*Pbh_array=NULL;
    PetscReal      Qw,Qw1,dervQw;
    PetscReal      dt,theta,G,effectiveCellPerm;
    PetscReal      CFbeta, CFalpha, CFgamma;
    ...
    for(i = 0; i < bag->WellinMeshData.numberWellsInProcessor; i++) {
        ...
        for(l = 0; l < bag->CVFEface.elemnodes; l++){
            ...
            G = 2*PETSC_PI*CFbeta*effectiveCellPerm*bag->well[w].h/(PetscLogReal(bag->well[w].re/bag->well[w].rw)+bag->well[w].sk);
            ierr = FracDQw(&Qw,bag->well[w],G,Pbh_array[w],P_array[l],Pb_array[l],Sw_array[l],Sg_array[l],Pcow_array[l],Pcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            ierr = FracDQw(&Qw1,bag->well[w],G,Pbh_array[w]+bag->SMALL_PRESSURE,P_array[l],Pb_array[l],Sw_array[l],Sg_array[l],Pcow_array[l],Pcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            dervQw = (Qw1-Qw)/bag->SMALL_PRESSURE;
            matvalue[l] = -1.*bag->epD.phi[l] * theta * dervQw;
        }
    ...
    /*
     PetscViewer viewer;
     ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"MatrixP3.txt",&viewer);CHKERRQ(ierr);
     ierr = MatView(K,viewer);CHKERRQ(ierr);
    */
    PetscFunctionReturn(0);
}
...
extern PetscErrorCode FracDRw(void *user, Vec Rw, Vec P, Vec Sw, Vec Sg, Vec Pbh)
{
    ...
    PetscInt       *closurept=NULL,*rclosurept=NULL,*rclosurept1=NULL;
    PetscReal      *cellcentroid,*edgecentroid,**facecentroid,**CVfacecoords,*wellcoords;;
    Vec            localResidual,localP,localPb,localoPb,localoP,localPcow,localoPcow,localPcog,localoPcog,localSw,localoSw,localSg,localoSg,localPbh,localoPbh,localQP;
    ...
    PetscScalar    *oP_array=NULL,*oPcow_array=NULL,*oPcog_array=NULL,*oSw_array=NULL,*oSg_array=NULL,*oPbh_array=NULL,*oPb_array=NULL;
    ...
    PetscReal      dt,theta,effectiveCellPerm;
    PetscReal      G,Bw,muw,phi,krw,lambda,fpress,Qw,Qw_o,kr_check,kr[2];
    PetscReal      Bw_o,muw_o,phi_o,krw_o,lambda_o,fpress_o,kr_check_o,kr_o[2];
    PetscReal      CFbeta, CFalpha, CFgamma;
    PetscErrorCode      (*FracDProjectFaceCoordinateDimensions)(PetscReal**,PetscReal**, PetscInt, PetscInt) = NULL;
    
    PetscFunctionBegin;
    ierr = VecSet(Rw,0.);CHKERRQ(ierr);
    switch (bag->dim) {
        case 2:
            FracDProjectFaceCoordinateDimensions = FracD1DProjectFaceCoordinateDimensions;
            break;
    ...
    ierr = DMGetLocalVector(bag->plexScalNode,&localoPb);CHKERRQ(ierr);
    ierr = DMGlobalToLocalBegin(bag->plexScalNode,bag->fields.oPb,INSERT_VALUES,localoPb);CHKERRQ(ierr);
    ierr = DMGlobalToLocalEnd(bag->plexScalNode,bag->fields.oPb,INSERT_VALUES,localoPb);CHKERRQ(ierr);
    ...
    for(c = cStart; c < cEnd; ++c){
        ...
        for(k = 0; k < numclpts; k++){
            ...
            if(pt >= gStart && pt < gEnd){
                ...
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr[0],Sw1_array[0],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr[1],Sw1_array[1],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr_o[0],oSw1_array[0],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                ierr = bag->ppties.RelPermData.FracDUpDateKrw(&kr_o[1],oSw1_array[1],PETSC_NULL,bag->ppties.RelPermData.Krw_TableData,PETSC_NULL,bag->ppties.RelPermData.numwaterdatarow);CHKERRQ(ierr);
                
                ierr =  bag->ppties.WaterPVTData.FracDUpDateFVF(&Bw,fpress,PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
                ierr =  bag->ppties.WaterPVTData.FracDUpDateFVF(&Bw_o,fpress_o,PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
                ierr =  bag->ppties.WaterPVTData.FracDUpDateViscosity(&muw,fpress,PETSC_NULL,bag->ppties.WaterPVTData.mu_TableData,bag->ppties.WaterPVTData.mu_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
                ierr =  bag->ppties.WaterPVTData.FracDUpDateViscosity(&muw_o,fpress_o,PETSC_NULL,bag->ppties.WaterPVTData.mu_TableData,bag->ppties.WaterPVTData.mu_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
                ierr = DMPlexGetTransitiveClosure(bag->plexScalNode, pt, PETSC_TRUE, &rnumclpts1, &rclosurept1);CHKERRQ(ierr);
                jj = 0;
                for(i = 0; i < rnumclpts1; i++){
                    rpt1 = rclosurept1[2*i];
                    ...
                    if(rpt1 >= vStart && rpt1 < vEnd){
                        ...
                        ierr = FracDDiffusiveFluxMatrixLocal(K_local, Permeability, vcoord_array, &bag->CVFEface);CHKERRQ(ierr);
                        for (cj = 0; cj < bag->CVFEface.elemnodes; cj++){
                            kr_check += K_local[cj] * (P_array[cj]-Pcow_array[cj]);
                            kr_check_o += K_local[cj] * (oP_array[cj]-oPcow_array[cj]);
                        }
                        if(kr_check > 0) krw = kr[jj];
                        else    krw = kr[(jj+1)%2];
                        if(kr_check_o > 0) krw_o = kr_o[jj];
                        else    krw_o = kr_o[(jj+1)%2];
                        lambda = krw/(muw*Bw);
                        lambda_o = krw_o/(muw_o*Bw_o);

                        for (l = 0, li = 0; li < numValues; li++) {
                            Residual_array[li] = 0;
                            for (cj = 0; cj < bag->CVFEface.elemnodes; cj++, l++) {
                                Residual_array[li] += CFbeta * K_local[l] * (theta*lambda*(P_array[cj]-Pcow_array[cj])+(1.-theta)*lambda_o*(oP_array[cj]-oPcow_array[cj]));
                            }
                        }
                        ierr = DMPlexVecRestoreClosure(cdm, cordSection, coordinates, rpt1, NULL, &vcoord_array);CHKERRQ(ierr);
        ...
        for(i = 0; i < bag->CVFEface.elemnodes; i++){
            Residual1_array[i] = 0;
            ierr = bag->ppties.WaterPVTData.FracDUpDateFVF(&Bw,P_array[i],PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
            ierr = bag->ppties.WaterPVTData.FracDUpDateFVF(&Bw_o,oP_array[i],PETSC_NULL,bag->ppties.WaterPVTData.B_TableData,bag->ppties.WaterPVTData.B_ModelData,bag->ppties.WaterPVTData.numdatarow);CHKERRQ(ierr);
            ierr = FracDInterpolateUsingAnalyticalModel(&phi,P_array[i],PETSC_NULL,PETSC_NULL,bag->ppties.PhiData,PETSC_NULL);CHKERRQ(ierr);
            ...
            Residual1_array[i] = 1./CFalpha * scale * bag->CVFEface.elemVolume * ((1./dt) * ((phi*Sw_array[i]/Bw)-(phi_o*oSw_array[i]/Bw_o))-QP_array[i]);
        }
    ...
    for(i = 0; i < bag->WellinMeshData.numberWellsInProcessor; i++) {
        ...
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localP, c, NULL, &P_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localPb, c, NULL, &Pb_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localoPb, c, NULL, &oPb_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localPcow, c, NULL, &Pcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localoPcow, c, NULL, &oPcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localPcog, c, NULL, &Pcog_array);CHKERRQ(ierr);
        ierr = DMPlexVecGetClosure(bag->plexScalNode, scalSection, localoPcog, c, NULL, &oPcog_array);CHKERRQ(ierr);
        ...
        for(l = 0; l < bag->CVFEface.elemnodes; l++){
            ...
            G = 2*PETSC_PI*CFbeta*effectiveCellPerm*bag->well[w].h/(PetscLogReal(bag->well[w].re/bag->well[w].rw)+bag->well[w].sk);
            ierr = FracDQw(&Qw,bag->well[w],G,Pbh_array[w],P_array[l],Pb_array[l],Sw_array[l],Sg_array[l],Pcow_array[l],Pcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            ierr = FracDQw(&Qw_o,bag->well[w],G,oPbh_array[w],oP_array[l],oPb_array[l],oSw_array[l],oSg_array[l],oPcow_array[l],oPcog_array[l],bag->ppties.WaterPVTData,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.RelPermData);CHKERRQ(ierr);
            Residual1_array[l] = -1. * bag->epD.phi[l] * (theta * Qw + (1-theta) * Qw_o);
        }
        ierr = DMPlexVecSetClosure(bag->plexScalNode, scalSection, localResidual, c, Residual1_array, ADD_ALL_VALUES);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexVecCell, matvecSection, local_perm, c, NULL, &  Perm_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localP, c, NULL, &P_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localPb, c, NULL, &Pb_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localoPb, c, NULL, &oPb_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localPcow, c, NULL, &Pcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localoPcow, c, NULL, &oPcow_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localPcog, c, NULL, &Pcog_array);CHKERRQ(ierr);
        ierr = DMPlexVecRestoreClosure(bag->plexScalNode, scalSection, localoPcog, c, NULL, &oPcog_array);CHKERRQ(ierr);
    ...
    ierr = DMLocalToGlobalBegin(bag->plexScalNode,localResidual,ADD_VALUES,Rw);CHKERRQ(ierr);
    ierr = DMLocalToGlobalEnd(bag->plexScalNode,localResidual,ADD_VALUES,Rw);CHKERRQ(ierr);
    ...
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localPb);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localoPb);CHKERRQ(ierr);
    ierr = DMRestoreLocalVector(bag->plexScalNode,&localPcow);CHKERRQ(ierr);
    ...
    if (bag->FlowFluxBC.hasLabel) {
        ierr = FracDResidualApplyCVFENeumannBC(Rw,dt,&bag->FlowFluxBC,&bag->elD,bag->FracDCreateDMinusOneFEElement,FracDProjectFaceCoordinateDimensions);CHKERRQ(ierr);
    }
    ierr = FracDResidualApplyDirichletBC(Rw,P,&bag->PBC);CHKERRQ(ierr);
    ...
    /*
     PetscViewer viewer;
     ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"Rw.txt",&viewer);CHKERRQ(ierr);
     ierr = VecView(Rw,viewer);CHKERRQ(ierr);
     ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD,"Rw1.txt",&viewer);CHKERRQ(ierr);
