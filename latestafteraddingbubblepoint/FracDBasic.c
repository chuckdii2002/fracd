...
extern PetscErrorCode FracDTimeStepPrepare(AppCtx *bag)
{
    ...
    bag->ppties.BubblePointFixed = bag->ppties.SolutionGasOilRatioFixed = bag->Sg_initial = 0.;
    ierr = PetscOptionsBegin(PETSC_COMM_WORLD, PETSC_NULL, "Initial state of the reservoir", "");CHKERRQ(ierr);
    {
        if (bag->fluid == OILWATERGAS){
            ierr = PetscOptionsReal("-initialbubblepoint","\n\t Initial bubble point pressure:","",bag->ppties.BubblePointFixed,&bag->ppties.BubblePointFixed,PETSC_NULL);CHKERRQ(ierr);
            ierr = PetscOptionsReal("-initialSg","\n\t Initial gas saturation:","",bag->Sg_initial,&bag->Sg_initial,PETSC_NULL);CHKERRQ(ierr);
        }
        ierr = bag->ppties.SolutionGasOilData.FracDUpDateSolutionGasOilRatio(&bag->ppties.SolutionGasOilRatioFixed,bag->ppties.BubblePointFixed,PETSC_NULL,bag->ppties.SolutionGasOilData.TableData,bag->ppties.SolutionGasOilData.ModelData,bag->ppties.SolutionGasOilData.numdatarow);CHKERRQ(ierr);
        bag->P_initial  = 0.;
    ...
    ierr = VecSet(bag->fields.oPb,bag->ppties.BubblePointFixed);CHKERRQ(ierr);
    ierr = VecSet(bag->fields.oRs,bag->ppties.SolutionGasOilRatioFixed);CHKERRQ(ierr);
