...
extern PetscErrorCode FracDProcessPVTData(AppCtx *bag)
{
    ...
    if (bag->fluid == OILWATERGAS){
        ...
        bag->ppties.SolutionGasOilData.FracDUpDateBubblePoint = FracDInterpolateUsingCubicSpline;
    }
    else{
        ...
        bag->ppties.SolutionGasOilData.FracDUpDateBubblePoint = FracDZeros;
    }
...
extern PetscErrorCode FracDCreateFields(AppCtx *bag,FracDFields *fields)
{
    ...
    ierr = PetscObjectSetName((PetscObject) fields->SgRs,"Combined_saturation_solution_variable");CHKERRQ(ierr);
    ierr = VecSet(fields->SgRs,0.0);CHKERRQ(ierr);
    
    ierr = DMCreateGlobalVector(bag->plexScalNode, &fields->oSgRs);CHKERRQ(ierr);
    ierr = PetscObjectSetName((PetscObject) fields->oSgRs,"Old_combined_saturation_solution_variable");CHKERRQ(ierr);
    ierr = VecSet(fields->oSgRs,0.0);CHKERRQ(ierr);
...
extern PetscErrorCode ThisismyVTKwriter(AppCtx *bag)
{
    ...
    ierr = VecView(bag->fields.SgRs,viewer);CHKERRQ(ierr);
...
extern PetscErrorCode FracDTimeStepPrepare(AppCtx *bag)
{
    ...
    {
        if (bag->fluid == OILWATERGAS){
            bag->ppties.SolutionGasOilData.isbubblepointvarying = PETSC_FALSE;
            ierr = PetscOptionsBool("-varyingbubblepoint", "fixed bubble point problem", " ", bag->ppties.SolutionGasOilData.isbubblepointvarying, &bag->ppties.SolutionGasOilData.isbubblepointvarying, PETSC_NULL);CHKERRQ(ierr);
    ...
    switch (bag->fluid) {
        ...
        case 3:
        {
            ierr = FracDInitializeSaturationBubblePointAndSwitchingVariables(bag->fields.INDC1,bag->fields.INDC2,bag->fields.Rs,bag->fields.P,bag->fields.Pb,bag->fields.Sw,bag->fields.Sg,bag->ppties.OilPVTData,bag->ppties.GasPVTData,bag->ppties.SolutionGasOilData);CHKERRQ(ierr);
            ierr = FracDInitializePackedSgRsField(bag->fields.SgRs,bag->fields.Rs,bag->fields.Sg,bag->fields.INDC2);CHKERRQ(ierr);
...
//            ierr = VecCopy(bag->fields.SgRs,X3);CHKERRQ(ierr);
            ierr = VecCopy(bag->fields.Pbh,X4);CHKERRQ(ierr);
            ierr = DMCompositeGetAccess(bag->MultiPhasePacker,bag->fields.FlowPacker,&X1,&X2,&X3,&X4);CHKERRQ(ierr);
        }
        break;
